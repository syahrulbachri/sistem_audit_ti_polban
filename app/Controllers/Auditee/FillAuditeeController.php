...
class FillAuditeeController extends BaseController
{
    ...
    public function saveAnswer(int $id)
    {
        ...
        if ($action === 'submit') {
            $total = $db->table('audit_question_assignments')->where('audit_id', $id)->countAllResults();
            ...
            if ($answered < $total) {
                return redirect()->to('/auditee/fill/' . $id)
                    ->with('error', 'Masih ada ' . ($total - $answered) . ' pertanyaan yang belum dijawab.');
            }
            ...
            // ✅ LOG AKTIVITAS: jawabannya dikirim ke auditor
            log_aktivitas('Mengirim jawaban audit "' . $audit->title . '" ke auditor (menunggu penilaian)', 'jawaban');
        ...
        log_aktivitas('Menyimpan draft jawaban audit "' . $audit->title . '"', 'jawaban');
        return redirect()->to('/auditee/fill/' . $id)
    ...
    public function revisi(int $id)
    {
        ...
        $audit = $db->table('audits')->where('id', $id)->where('auditee_id', $userId)->get()->getRow();
        if (!$audit)
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();

        // Ambil temuan yang masih terbuka + pertanyaan + jawaban saat ini
        $temuans = $db->table('temuans')
            ->select('temuans.*, aq.clause_code, aq.question_text, aqa.id as assignment_id, aqa.answer, aqa.evidence_filename')
            ->join('audit_questions as aq', 'aq.id = temuans.question_id', 'left')
            ->join('audit_question_assignments as aqa', 'aqa.audit_id = temuans.audit_id AND aqa.question_id = temuans.question_id', 'left')
    ...
    public function saveRevisi(int $id)
    {
        ...
        if (isset($files['evidence']) && is_array($files['evidence'])) {
            foreach ($files['evidence'] as $assignmentId => $file) {
                if ($file instanceof \CodeIgniter\HTTP\Files\UploadedFile && $file->isValid() && !$file->hasMoved()) {
                    ...
                    $uploadedNames[$assignmentId] = $file->getClientName();
                }
            }
        }

        // 3. KIRIM KE AUDITOR: ubah status temuan jadi In_Progress
        // [PERBAIKAN] Gabungkan ID dari jawaban teks dan upload file agar tidak ada yang terlewat
        $processedIds = array_unique(array_merge(array_keys($answers), array_keys($uploadedNames)));
        ...
        foreach ($processedIds as $assignmentId) {
            ...
            if ($assignment) {
                $dataTemuan = [
                    'status' => 'In_Progress',
                    'revisi_sent_at' => date('Y-m-d H:i:s'), // ← penanda revisi udah dikirim
                    'updated_at' => date('Y-m-d H:i:s')
                ];

                // Jika ada bukti baru, sync ke temuan
                if (isset($uploadedNames[$assignmentId])) {
                    $dataTemuan['bukti_perbaikan'] = $uploadedNames[$assignmentId];
                }

                // Update tabel temuans (hanya yang statusnya Open atau In_Progress)
                $db->table('temuans')
                    ->where('audit_id', $id)
        ...
        log_aktivitas('Mengirim revisi jawaban & bukti perbaikan audit "' . $audit->title . '" (menunggu verifikasi auditor)', 'revisi');

        return redirect()->to('/auditee/dashboard')
            ->with('success', 'Revisi berhasil dikirim ke auditor. Silakan tunggu verifikasi.');
    }
}
